...
class compras_presupuesto extends fs_controller {
   ...
   {
      parent::__construct(__CLASS__, ucfirst(FS_PRESUPUESTO), 'compras', FALSE, FALSE);
   }
   ...
   private function modificar()
   {
            ...
            if($proveedor)
            {
               foreach($proveedor->get_direcciones() as $d)
               {
                  if($d->domfacturacion)
                  {
                     ...
                     $this->presupuesto->proveedor = $proveedor->nombrecomercial;
      ...
      {
         $this->new_message(ucfirst(FS_PRESUPUESTO) . " modificado correctamente.");
         $this->new_change(ucfirst(FS_PRESUPUESTO) .' Proveedor '. $this->presupuesto->codigo, $this->presupuesto->url());
      }
      else
         $this->new_error_msg("¡Imposible modificar el " . FS_PRESUPUESTO . "!");
   }
